...
class LandingController extends AppBaseController
{
    ...
    public function campaignDetails(Campaign $campaign)
    {
        ...
        $sidebarCampaignCategories = CampaignCategory::withCount('campaigns')->with('campaigns')->whereHas(
            'campaigns',
            function (Builder $query) {
            }
        ...
        $donationEnableGifts = Campaign::with('campaignGifts')->where('id', '=', $campaign->id)->where(
            'gift_status',
            '=',
            true
        )->first();
    ...
    public function donation(Request $request)
    {
        ...
        $invest = InvestPackage::find($request->investment_id);
        $transaction['order_id'] = uniqid(); // invoice number
        $transaction['amountTotal'] = (FLOAT) $request->amount;
        $transaction['note'] = 'Inscription au Package '.$invest->title;
        $transaction['buyer_name'] = $request->first_name.' '.$request->last_name;
        $transaction['buyer_email'] = $request->email;
        $transaction['redirect_url'] = route('user.dashboard.index', $invest->slug); // When Transaction was comleted
        $transaction['cancel_url'] = route('landing.payment', $invest->slug);

        $transaction['items'][] = [
            'itemDescription' => 'Don pour le projet: '.$invest->title,
    ...
    public function projectWizard(){
        ...
        return view('user.campaigns.wizard',compact('campaignCategory'));
    }
}
